...
Graph createFlowChart(const clang::FunctionDecl* fdecl)
{
    ...
    auto start              = addFlowchartVertex(g, new VertexBegin(), 0,
                                    functionName + "\n" + RUK_FUNCTION_BEGIN);
    auto end = addFlowchartVertex(g, new VertexEnd(), 0,
                                  functionName + "\n" + RUK_FUNCTION_END);
    auto body =
        getSemanticVertexFromStmt(fdecl->getBody(), g, fdecl->getASTContext());
...
void dumpGraph(clang::ASTContext& Context)
{
    ...
    //       boost::write_graphviz( std::cout, bigGraph, myLabler( bigGraph ),
    //       myEdgeLabler(bigGraph), myGraphPropertyWriter() );
    ...
    for (const auto& d : functionDeclsInMain) {
        ...
        for (const auto p : getFunctionParams(d))
            std::cout << p->getNameAsString() << " : "
                      << p->getType().getAsString() << ";" << std::endl;
        std::cout << std::endl;
    }
}
...
int printFlowChart(int argc, char** argv)
{
    ...
    for (const auto& name : filenames) {
        ...
        runToolOnCodeWithArgs(new FlowChartAction(), code.c_str(), params,
                              name.c_str());
    }
    return 0;
}
