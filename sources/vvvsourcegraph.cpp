...
getSemanticVertexFromStmt(const Stmt* stmt, Graph& graph,
                          const clang::ASTContext& context)
{
    ...
    if (stmt)
        ...
        case Stmt::CompoundStmtClass:
            ret = new BlockCompound(graph, stmt, context);
            break;
        case Stmt::IfStmtClass: ret = new BlockIf(graph, stmt, context); break;
        case Stmt::SwitchStmtClass:
            ret = new BlockSwitch(graph, stmt, context);
            break;
        case Stmt::CaseStmtClass:
            ret = new BlockCase(graph, stmt, context);
            break;
        case Stmt::DefaultStmtClass:
            ret = new BlockDefault(graph, stmt, context);
            break;
        case Stmt::ForStmtClass:
            ret = new BlockFor(graph, stmt, context);
            break;
        case Stmt::WhileStmtClass:
            ret = new BlockWhile(graph, stmt, context);
            break;
        case Stmt::DoStmtClass:
            ret = new BlockDoWhile(graph, stmt, context);
            break;
        case Stmt::CallExprClass: {
            auto ce = static_cast<const clang::CallExpr*>(stmt);
            if (isSystemDecl(ce->getCalleeDecl()))
                ret = new BlockSimple(graph, stmt, context);
            else
                ret = new BlockCall(graph, stmt, context);
            break;
        }
        case Stmt::ReturnStmtClass:
            ret = new BlockReturn(graph, stmt, context);
            break;
...
vertex_t BlockIf::expand(vertex_t begin, vertex_t end, vertex_t onReturn,
                         vertex_t onBreak, vertex_t onContinue, int depth)
{
    ...
    auto thenVertex =
        thenStmt->expand(vertex, end, onReturn, onBreak, onContinue, depth + 1);
    edge_t trueBranch = boost::add_edge(vertex, thenVertex, graph).first;
    edge_t falseBranch;
    if (elseStmt) {
        auto elseVertex = elseStmt->expand(vertex, end, onReturn, onBreak,
                                           onContinue, depth + 1);
...
vertex_t BlockCall::expand(vertex_t begin, vertex_t end, vertex_t onReturn,
                           vertex_t onBreak, vertex_t onContinue, int depth)
{
...
vertex_t BlockReturn::expand(vertex_t begin, vertex_t end, vertex_t onReturn,
                             vertex_t onBreak, vertex_t onContinue, int depth)
{
    ...
    const std::string contents =
        std::string("return ") + decl2str(stmt->getRetValue(), context);
    addToTable(LABEL_TYPE::OPERATOR, contents);
    return vertex;
}

vertex_t BlockSimple::expand(vertex_t begin, vertex_t end, vertex_t onReturn,
                             vertex_t onBreak, vertex_t onContinue, int depth)
{
    vertex = addFlowchartVertex(graph, new VertexProcess(), depth);
    boost::add_edge(vertex, end, graph);
...
groupChildren(const std::vector<const Stmt*>& stmts, Graph& graph,
              const ASTContext& context)
{
    ...
    auto addOneBlock = [&oneBlock, &ret, &graph, &context]() {
        if (oneBlock.size()) {
            ret.push_back(
                uptrSemVert(new BlockSimpleCompound(graph, oneBlock, context)));
            oneBlock.clear();
        }
    };
    ...
    for (auto& stmt : stmts) {
        ...
        if (sv->getType() != SEMANTIC_BLOCK_TYPE::SIMPLE) {
            addOneBlock(); // add collected simple statements as one statement
                           // if any exist
            ret.push_back(std::move(sv));
        }
        else
            oneBlock.push_back(std::move(sv));
    } // add statement to
    addOneBlock();
...
vertex_t BlockCompound::expand(vertex_t begin, vertex_t end, vertex_t onReturn,
                               vertex_t onBreak, vertex_t onContinue, int depth)
{
    ...
    case 1:
        groupedChildren[0]->expand(begin, end, onReturn, onBreak, onContinue,
                                   depth);
        break;
    default: {
        ...
        groupedChildren[i]->expand(begin, end, onReturn, onBreak, onContinue,
                                   depth);
        while (i-- != 0)
            groupedChildren[i]->expand(begin,
                                       groupedChildren[i + 1]->getVertex(),
                                       onReturn, onBreak, onContinue, depth);
        break;
    }
    };

    vertex = groupedChildren[0]
                 ->getVertex(); // TODO: if compound block empty there are BUG
    return vertex;
}

vertex_t BlockSimpleCompound::expand(vertex_t begin, vertex_t end,
                                     vertex_t onReturn, vertex_t onBreak,
                                     vertex_t onContinue, int depth)
{
    vector<string> v;
...
vertex_t BlockFor::expand(vertex_t begin, vertex_t end, vertex_t onReturn,
                          vertex_t onBreak, vertex_t onContinue, int depth)
{
    ...
    auto endLoopVertex =
        addFlowchartVertex(graph, new VertexLoopClose(vertex), depth);
    boost::add_edge(endLoopVertex, end, graph);

    // expand loop body
    auto body = getSemanticVertexFromStmt(stmt->getBody(), graph, context);
    auto bodyVertex = body->expand(vertex, endLoopVertex, onReturn, end,
                                   endLoopVertex, depth + 1);
...
vertex_t BlockWhile::expand(vertex_t begin, vertex_t end, vertex_t onReturn,
                            vertex_t onBreak, vertex_t onContinue, int depth)
{
    const std::string contents = decl2str(stmt->getCond(), context);
...
vertex_t BlockDoWhile::expand(vertex_t begin, vertex_t end, vertex_t onReturn,
                              vertex_t onBreak, vertex_t onContinue, int depth)
{
    const std::string contents =
        std::string("do while: ") + decl2str(stmt->getCond(), context);
...
vertex_t BlockSwitch::expand(vertex_t begin, vertex_t end, vertex_t onReturn,
                             vertex_t onBreak, vertex_t onContinue, int depth)
{
    const std::string contents =
        std::string("switch: ") + decl2str(stmt->getCond(), context);
    ...
    case 1:
        groupedChildren[0]->expand(vertex, end, onReturn, end, onContinue,
                                   depth + 1);
        boost::add_edge(vertex, groupedChildren[0]->getVertex(), graph);
        break;
    default: {
        size_t i = numChildren - 1;
        ...
        while (i-- != 0) {
            ...
            const auto localend =
                (nextChild->getType() == SEMANTIC_BLOCK_TYPE::BREAK)
                    ? end
                    : nextChild->getVertex();
            ...
            case SEMANTIC_BLOCK_TYPE::DEFAULT:
                currentChild->expand(vertex, localend, onReturn, end,
                                     onContinue, depth + 1);
                break;
            default:
                currentChild->expand(vertex, localend, onReturn, end,
...
const std::pair<vector<string>, const Stmt*>& BlockCase::getConditions() const
{
    if (conditions.first.empty()) {
        ...
        while (currentStmt) {
            ...
            switch (stmtClass) {
            case Stmt::CaseStmtClass:
                conditions.first.push_back(decl2str(
                    static_cast<const CaseStmt*>(currentStmt)->getLHS(),
                    context));
                currentStmt =
                    static_cast<const CaseStmt*>(currentStmt)->getSubStmt();
                break;
            case Stmt::DefaultStmtClass:
                conditions.first.push_back("default");
                currentStmt =
                    static_cast<const DefaultStmt*>(currentStmt)->getSubStmt();
                break;
            default:
...
vertex_t BlockCase::expand(vertex_t begin, vertex_t end, vertex_t onReturn,
                           vertex_t onBreak, vertex_t onContinue, int depth)
{
    const auto& condition = getConditions();

    auto statements =
        getSemanticVertexFromStmt(condition.second, graph, context);
    vertex =
        statements->expand(begin, end, onReturn, onBreak, onContinue, depth);

    boost::remove_edge(begin, vertex,
                       graph); // TODO: TODO: TODO: TODO:TODO: TODO
    auto beginvertex = boost::add_edge(begin, vertex, graph)
                           .first; // TODO: TODO: TODO: TODO:TODO: TODO
    graph[beginvertex].text =
        joinStringsWith(condition.first, ", "); // do it from switch

    return vertex;
}

vertex_t BlockDefault::expand(vertex_t begin, vertex_t end, vertex_t onReturn,
                              vertex_t onBreak, vertex_t onContinue, int depth)
{
    auto statements =
        getSemanticVertexFromStmt(stmt->getSubStmt(), graph, context);
    vertex =
        statements->expand(begin, end, onReturn, onBreak, onContinue, depth);
...
vertex_t BlockBreak::expand(vertex_t begin, vertex_t end, vertex_t onReturn,
                            vertex_t onBreak, vertex_t onContinue, int depth)
{
    vertex = onBreak;
    return onBreak;
...
vertex_t BlockContinue::expand(vertex_t begin, vertex_t end, vertex_t onReturn,
                               vertex_t onBreak, vertex_t onContinue, int depth)
{
    vertex = onContinue;
    return onContinue;
...
static int branchesnesRecursion(const Graph& g, vertex_t v,
                                vertex_t limitVertex)
{
