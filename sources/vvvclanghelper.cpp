...
std::vector<const RecordDecl*> filterStructs(const std::vector<const Decl*>& decls)
{
    std::vector<const RecordDecl*> ret;
    const auto fd  = filter( decls, [](const Decl* d) { return d->getKind() == clang::Decl::Kind::Record; });
    for( const auto& d: fd )
